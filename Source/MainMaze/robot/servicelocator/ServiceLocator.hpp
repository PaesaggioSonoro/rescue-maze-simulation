...
public:
    ...
    public:
        Context(Context* root, Context* parent, wptr<ServiceLocator> sl, const std::type_index interfaceType,
                const std::string& name) : _root(root), _parent(parent), _sl(sl), _interfaceType(interfaceType),
                                           _name(name)
        {
        }

        Context(Context* parent, const std::type_index interfaceType, const std::string& name) : Context(
            parent->_root, parent, parent->_sl, interfaceType, name)
        {
        }

        Context(wptr<ServiceLocator> sl, const std::type_index interfaceType, const std::string& name) : Context(
            this, nullptr, sl, interfaceType, name)
        {
        }
        ...
        if (nsl == nullptr)
        {
            if (_parent == nullptr)
            {
                throw UnableToResolveException(
                    std::string("Unable to resolve <") + slc->getInterfaceTypeName() + ">  resolve path = " + slc->
                    getResolvePath());
            }
